...
export default function WalletModal({ wallet, onClose, onDeposit, onWithdraw, openTrades = [] }) {
  ...
  const TabBtn = ({ id, icon, label }) => (
    <button onClick={() => { setTab(id); setFeedback(null); setAmount('') }} style={{
      ...
      background: tab === id ? '#0d0d28' : 'transparent',
      border:'none', borderBottom:`2px solid ${tab === id ? '#38bdf8' : 'transparent'}`,
      color: tab === id ? '#38bdf8' : '#4a4a7a',
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:250, background:'rgba(1,1,8,0.93)', backdropFilter:'blur(6px)',
      ...
      `}</style>
      <div style={{ width:'100%', maxWidth:390, maxHeight:'91vh', overflowY:'auto',
        ...
        <div style={{ padding:'16px 20px 10px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
          <div style={{ background:'#06060f', border:'1px solid #1a1a38', borderRadius:10, padding:12 }}>
          ...
          <div style={{ background:'#06060f', border:`1px solid ${totalPnl >= 0 ? '#22c55e28' : '#ef444428'}`, borderRadius:10, padding:12 }}>
        ...
        <div style={{ padding:'18px 20px 26px' }}>
          ...
          {tab === 'overview' && (
            <div>
              ...
              {openTrades.length === 0 ? (
                ...
                <div key={t.id} style={{ background:'#07071a', border:`1px solid ${(t.trade_type ?? t.type) === 'BUY' ? '#22c55e20' : '#ef444420'}`, borderRadius:10, padding:'11px 13px', marginBottom:8 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:7 }}>
                    <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                      <span style={{ color:(t.trade_type ?? t.type) === 'BUY' ? '#4ade80' : '#f87171', fontSize:11, fontWeight:'bold', letterSpacing:1 }}>
                        {(t.trade_type ?? t.type) === 'BUY' ? '▲' : '▼'} {t.trade_type ?? t.type}
                      </span>
                      <span style={{ color:'#a78bfa', fontSize:10, letterSpacing:1 }}>{t.symbol}</span>
                    </div>
                    <div style={{ textAlign:'right' }}>
                      <div style={{ color:(t.pnl ?? 0) >= 0 ? '#4ade80' : '#f87171', fontSize:12, fontWeight:'bold' }}>
                        {(t.pnl ?? 0) >= 0 ? '+' : ''}{fmt(t.pnl ?? 0)}
                      </div>
                    </div>
                  </div>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:4, fontSize:8, marginBottom:6 }}>
                    <div><span style={{ color:'#2a2a50' }}>ENTRY </span><span style={{ color:'#38bdf8' }}>{t.entry_price ?? t.entryStr ?? '–'}</span></div>
                    <div><span style={{ color:'#2a2a50' }}>TP </span><span style={{ color:'#4ade80' }}>{t.take_profit ?? t.tpStr ?? '–'}</span></div>
                    <div><span style={{ color:'#2a2a50' }}>SL </span><span style={{ color:'#f87171' }}>{t.stop_loss ?? t.slStr ?? '–'}</span></div>
                  </div>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span style={{ color:'#2e2e58', fontSize:7.5 }}>{t.lot_size ?? t.lot} · Vol {t.volume ?? t.vol}</span>
                    <button onClick={() => {}} style={{ background:'#0f0f28', border:'1px solid #3a2a60', color:'#9080c0',
              ...
              {openTrades.length > 0 && (
                <button style={{ width:'100%', marginTop:6, padding:9, borderRadius:8,
              ...
              <div style={{ display:'flex', gap:8, marginTop:14 }}>
                <button onClick={() => setTab('deposit')} style={{ flex:1, padding:11, borderRadius:8,
                  background:'#06101a', border:'1.5px solid #38bdf8', color:'#38bdf8',
                  fontFamily:'inherit', fontSize:10, fontWeight:'bold', letterSpacing:2, cursor:'pointer' }}>
                  ↓ DEPOSIT
                </button>
                <button onClick={() => setTab('withdraw')} style={{ flex:1, padding:11, borderRadius:8,
          ...
          {tab === 'deposit' && (
            <div>
              ...
              <button onClick={doDeposit} style={{ width:'100%', padding:13, borderRadius:10,
          ...
          {tab === 'withdraw' && (
            <div>
              ...
              <button onClick={doWithdraw} disabled={openTrades.length > 0} style={{ width:'100%', padding:13, borderRadius:10,
